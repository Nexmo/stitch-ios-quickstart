...
internal struct TaskDAO: CRUD {
    ...
    /// - Parameters:
    ///   - type: tpye of task
    ///   - eventId: event id and covnersation id
    /// - Returns: result
    internal func taskExist(_ type: DBTask.OperationType, for eventId: String) -> Bool {
        return database.queue.inDatabase {
            T.all()
                .filter(sql: "type = \(type.rawValue) AND related = '\(eventId)'")
                .fetchCount($0) >= 1
        }
    }
}
