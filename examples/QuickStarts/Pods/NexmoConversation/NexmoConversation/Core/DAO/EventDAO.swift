...
internal struct EventDAO: CRUD {
    ...
    /// - Returns: type T
    internal subscript(with id: String, in uuid: String) -> T? {
    ...
    internal func getDraft(_ id: String, in uuid: String) -> T? {
        return database.queue.inDatabase { database in
            return T
                .all()
                .filter(sql: "isDraft = ? AND cid = ? AND eventId = ?", arguments: [true, uuid, id])
                .fetchOne(database)
        }
    }
    ...
    /// - Returns: success/fail
    /// - Throws: CRUDErrors
    internal func lastEventId(in uuid: String) -> Int64? {
        return database.queue.inDatabase {
            Int64.fetchOne($0, T.select(sql: "MAX(eventId)").filter(sql: "cid = ?", arguments: [uuid]))
        }
    }
}
