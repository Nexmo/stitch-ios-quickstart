...
public class Member: NSObject {
    ...
    public var user: User {
        guard let local = ConversationClient.instance.storage.userCache.get(uuid: data.rest.userId) else {
            return User(from: UserModel(displayName: "", imageUrl: "", uuid: data.rest.userId, name: data.rest.name))
        }
    ...
    public var conversation: Conversation { return ConversationClient.instance.storage.conversationCache.get(uuid: data.parent)! }

    /// The member who invited you
    public var invitedBy: Member? {
        return ConversationClient.instance.storage.memberCache
            .all()
            .flatMap { $0 }
