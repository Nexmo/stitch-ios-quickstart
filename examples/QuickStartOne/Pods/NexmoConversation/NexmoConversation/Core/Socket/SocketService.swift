...
internal class SocketService {
    ...
    internal enum Event: String {
        ...
        case userNotFound = "user:error:not-found"
    }
    ...
    private func setup() {
        ...
        webSocketManager.on(Event.userNotFound.rawValue) { self.updateState(with: .notConnected(.unknown($0))) }
    ...
    internal func validate(_ response: [Any]) -> Bool {
        guard let model = try? JSONDecoder().decode(Session.self, from: response.first as? [String: AnyObject]) else { return false }
