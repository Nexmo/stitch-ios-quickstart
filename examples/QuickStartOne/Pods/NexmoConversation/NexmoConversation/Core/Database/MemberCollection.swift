...
public class MemberCollection: NexmoConversation.LazyCollection<Member> {
    ...
    private func setup(_ event: EventBase) {
    ...
    public override subscript(uuid: String) -> Member? {
        return ConversationClient.instance.storage.memberCache.get(uuid: uuid)
    }
    ...
    public override subscript(i: Int) -> Member {
        return ConversationClient.instance.storage.memberCache.get(uuid: uuids[i])!
    }
}
