...
extension Database {
            ...
            case .release:
                ...
                if savepointStack.isEmpty {
                    ...
                    for (event, notifiedObservers) in eventsBuffer {
                        for observer in notifiedObservers.compactMap({ $0.transactionObserver }) {
    ...
    private func willCommit() throws {
        ...
        var notifiedObserversForCommit: [TransactionObserver] = databaseEventObservers.compactMap({ $0.transactionObserver })
        for (event, notifiedObservers) in eventsBuffer {
            for observer in notifiedObservers.compactMap({ $0.transactionObserver }) {
                event.send(to: observer)
    ...
    private func didChange(with event: DatabaseEvent) {
        if savepointStack.isEmpty {
            ...
            // Only notify those that are interested in the event, and have been
            // isolated in updateStatementWillExecute().
            for observer in statementEventObservers.compactMap({ $0.transactionObserver }) {
    ...
    private func didCommit() {
        for observer in databaseEventObservers.compactMap({ $0.transactionObserver }) {
